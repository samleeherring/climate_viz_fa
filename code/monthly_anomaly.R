...
p <- t_data %>%
  ...
  labs(
    ...
    subtitle = "(Difference from 1980-2024 annual mean)"
  )+
...
p +
  geom_point(data = annotation, aes(x=month, y=month_anom), size = 3)+
  geom_text(data = annotation, aes(x=month, y=month_anom),
            label = "March 2024", vjust = -1.15)
